...
describe('Master service', () => {
  it('canonicalizes vendor code and writes audit', async () => {
    ...
    const result = await service.createVendor(actor, { code: 'pt topabiring', name: 'PT Topabiring', aliases: [], contactEmail: null }, 'req-1');
  ...
  it('requires reason when active status changes', async () => {
    const service = createMasterService(repo({
      findVendorById: async () => ({ id: '20000000-0000-4000-8000-000000000001', code: 'V1', name: 'Vendor 1', aliases: [], contactEmail: null, active: true, createdAt: now, updatedAt: now }),
    }));
  ...
  it('blocks duplicate equipment business key per vendor/type/unit', async () => {
    ...
    const service = createMasterService(repo({
      findVendorById: async () => ({ id: vendorId, code: 'V1', name: 'Vendor 1', aliases: [], contactEmail: null, active: true, createdAt: now, updatedAt: now }),
      findEquipmentByBusinessKey: async () => ({ id: '40000000-0000-4000-8000-000000000001', vendorId, vendorCode: 'V1', vendorName: 'Vendor 1', type: 'AA', unitNo: '12', brand: null, model: null, aliases: [], active: true, createdAt: now, updatedAt: now }),
    }));
    await expect(service.createEquipment(actor, { vendorId, type: 'AA', unitNo: '12', aliases: [] })).rejects.toMatchObject({ code: 'EQUIPMENT_EXISTS' });
  });

  it('limits vendor lookup to the logged-in vendor', async () => {
    const vendorA = { id: '50000000-0000-4000-8000-000000000001', code: 'A', name: 'Vendor A', aliases: [], contactEmail: null, active: true, createdAt: now, updatedAt: now };
    const vendorB = { id: '50000000-0000-4000-8000-000000000002', code: 'B', name: 'Vendor B', aliases: [], contactEmail: null, active: true, createdAt: now, updatedAt: now };
  ...
  it('blocks a vendor alias already owned by another canonical vendor', async () => {
    const service = createMasterService(repo({
      findVendorByAlias: async () => ({ id: '60000000-0000-4000-8000-000000000001', code: 'OLD', name: 'Existing Vendor', aliases: ['PT TEST'], contactEmail: null, active: true, createdAt: now, updatedAt: now }),
    }));
    await expect(service.createVendor(actor, { code: 'NEW', name: 'New Vendor', aliases: [' pt   test '], contactEmail: null })).rejects.toMatchObject({ code: 'VENDOR_ALIAS_EXISTS' });
  });
