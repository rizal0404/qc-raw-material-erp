...
import { aggregateStockpileMixes, stockpileQualityStatus, validateStockpileRectangle } from '@qc/domain';
...
export function createStockpileMapService(repository:StockpileMapRepository,masterRepository:MasterRepository,qcRepository:QcRepository){
  ...
  return{
    ...
    async createLayer(principal:AuthPrincipal,input:CreateStockpileLayerRequest,requestId?:string){
      assertQc(principal);const layout=await layoutOrThrow(input.layoutId);const rectangle=validateStockpileRectangle(layout,input);const mixes=await repository.findMixes(input.mixIds);if(mixes.length!==input.mixIds.length)throw notFound('Satu atau lebih Mix aktif tidak ditemukan.');
