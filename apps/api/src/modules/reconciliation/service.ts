...
function derivedStatus(a:ReconciliationAssignmentRecord,candidateCount:number):MappingStatus{
  ...
  if(a.allocations.some(x=>x.mappingStatus==='RESERVED')||candidateCount===1)return 'RESERVED';
  return 'AMBIGUOUS';
}
function exceptionOf(a:ReconciliationAssignmentRecord,status:MappingStatus,candidates:number):string|null{
  ...
  if(status==='UNMAPPED')return 'Tidak ada Sample_ID candidate berdasarkan afiliasi AM/Vendor.';
  if(status==='AMBIGUOUS'||candidates>1)return `${candidates} Sample_ID candidate berdasarkan afiliasi AM/Vendor.`;
...
export function createReconciliationService(repository:ReconciliationRepository){
  async function assignmentWithCandidates(a:ReconciliationAssignmentRecord,principal:AuthPrincipal){
    const candidates=await repository.listCandidates(a.assignmentId);
    let allocations=a.allocations;
    const protectedAllocation=a.allocations.some(x=>x.reviewRequired||['CONFIRMED','CONSUMED','REVIEW_REQUIRED'].includes(x.mappingStatus));
    if(candidates.length===1&&!protectedAllocation){
      const result=await repository.autoReserveSingleCandidate({assignmentId:a.assignmentId,sampleId:candidates[0]!.id,observedRetase:a.observedRetase,createdBy:principal.userId});
      if(result){
        allocations=a.allocations.some(x=>x.id===result.allocation.id)
          ? a.allocations.map(x=>x.id===result.allocation.id?result.allocation:x)
          : [...a.allocations,result.allocation];
        if(result.changed)await repository.appendAudit({actorUserId:principal.userId,actorRoleSnapshot:principal.role,action:'AUTO_RESERVE',entityType:'QC_RETASE_ALLOCATION',entityId:result.allocation.id,afterJson:{assignmentId:a.assignmentId,sampleId:result.allocation.sampleId,approvedRetase:result.allocation.approvedRetase,candidateCount:1},reason:'Satu raw sample candidate terafiliasi ke vendor assignment.',});
      }
    }
    const reservedRetase=allocations.reduce((total,x)=>total+(x.mappingStatus==='CONSUMED'?x.consumedRetase:['RESERVED','CONFIRMED','REVIEW_REQUIRED'].includes(x.mappingStatus)?(x.approvedRetase??0):0),0);
    const consumedRetase=allocations.reduce((total,x)=>total+x.consumedRetase,0);
    const current={...a,reservedRetase,consumedRetase,remainingRetase:a.observedRetase-reservedRetase,allocations};
    const status=derivedStatus(current,candidates.length);const suggested=candidates.length===1?candidates[0]!:null;
    return {...current,candidateCount:candidates.length,suggestedSampleId:suggested?.id??null,suggestedSampleCode:suggested?.sampleId??null,mappingStatus:status,reviewRequired:current.reviewRequired||allocations.some(x=>x.reviewRequired),exception:exceptionOf(current,status,candidates.length),allocations:allocations.map(allocationDto)};
  }
  ...
  return {
    async list(principal:AuthPrincipal,query:ReconciliationListQuery){
      ...
      const base=await repository.listAssignments(query);const items=[];for(const a of base){const x=await assignmentWithCandidates(a,principal);if(!query.mappingStatus||x.mappingStatus===query.mappingStatus)items.push(x);}
    ...
    async createAllocation(principal:AuthPrincipal,input:CreateRetaseAllocationRequest,requestId?:string){
      ...
      const candidates=await repository.listCandidates(a.assignmentId);if(!candidates.some(c=>c.id===input.sampleId))throw new AppError(400,'SAMPLE_NOT_CANDIDATE','Sample_ID tidak termasuk candidate berdasarkan afiliasi AM/Vendor.');
    ...
    async updateAllocation(principal:AuthPrincipal,id:string,input:UpdateRetaseAllocationRequest,requestId?:string){
      ...
      const a=await repository.getAssignment(current.assignmentId);if(!a)throw notFound('Loading assignment tidak ditemukan.');assertLimestone(a.materialKind);const candidates=await repository.listCandidates(a.assignmentId);const sampleId=input.sampleId??current.sampleId;if(!sampleId||!candidates.some(c=>c.id===sampleId))throw new AppError(400,'SAMPLE_NOT_CANDIDATE','Sample_ID tidak termasuk candidate berdasarkan afiliasi AM/Vendor.');
