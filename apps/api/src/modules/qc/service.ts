...
import type { AuthPrincipal, MasterRepository, MixSummaryRecord, QcRepository, RawSampleRecord, RawSampleWriteInput } from '@qc/domain';
...
export function createQcService(repository: QcRepository, masterRepository: MasterRepository) {
  ...
  async function resolveReferences(input: SampleReferenceInput, current?: RawSampleRecord): Promise<Partial<RawSampleWriteInput>> {
    ...
    if ('vendorId' in input) {
      if (input.vendorId) { const v=await masterRepository.findVendorById(input.vendorId); if(!v)throw notFound('Vendor tidak ditemukan.'); out.vendorId=v.id;out.vendorSnapshot=input.vendorSnapshot ?? v.name; }
    ...
    if ('plantId' in input) {
      if(input.plantId){const p=await masterRepository.findPlantById(input.plantId);if(!p)throw notFound('Plant tidak ditemukan.');out.plantId=p.id;}else out.plantId=null;
    }
    return out;
  }
  ...
  async function prepareMix(actor:AuthPrincipal,input:SaveMixRequest) {
    ...
    const duplicate=new Set<string>(); const items=[] as Array<{rawSampleId:string;retase:number;tonPerRetase:number;chemistry:Chemistry;note:string|null;oxideChangeNote:string|null;retaseAllocationIds:string[];retaseOverrideReason:string|null}>;
    for(const item of input.items){if(duplicate.has(item.rawSampleId))throw new AppError(400,'DUPLICATE_SAMPLE','Sample yang sama hanya boleh satu kali dalam mix.');duplicate.add(item.rawSampleId);const raw=await repository.findRawSampleById(item.rawSampleId);if(!raw)throw notFound('Raw sample pada mix tidak ditemukan.');if(raw.materialKind!==input.materialKind)throw new AppError(400,'MATERIAL_KIND_MISMATCH',`Sample ${raw.sampleId} berbeda material.`);if(raw.operationDate!==input.operationDate)throw new AppError(400,'SAMPLE_DATE_MISMATCH',`Tanggal sample ${raw.sampleId} berbeda dari Operation Date mix.`);const allocationIds=[...new Set(item.retaseAllocationIds??[])];if(item.retase>0&&!allocationIds.length&&await repository.hasConfirmedRetaseAllocation(item.rawSampleId,input.operationDate))throw conflict('MAPPED_RETASE_REQUIRED',`Sample ${raw.sampleId} memiliki confirmed mapped retase. Apply mapping dari Reconciliation sebelum Save Mix.`);const chemistry=item.chemistry ?? raw.chemistry;const changed=!chemistryEquals(raw.chemistry,chemistry);if(changed&&!item.oxideChangeNote)throw new AppError(400,'OXIDE_NOTE_REQUIRED',`Note perubahan oksida wajib untuk ${raw.sampleId}.`);items.push({rawSampleId:item.rawSampleId,retase:item.retase,tonPerRetase:item.tonPerRetase,chemistry,note:nullable(item.note),oxideChangeNote:changed?(item.oxideChangeNote??null):null,retaseAllocationIds:allocationIds,retaseOverrideReason:nullable(item.retaseOverrideReason)});}
  ...
  return {
    ...
    async importSamples(actor:AuthPrincipal,input:ImportRawSamplesRequest,requestId?:string){const vendorCache=new Map<string,ReturnType<MasterRepository['findVendorById']>>(),sourceCache=new Map<string,ReturnType<MasterRepository['findSourceById']>>();const rows:RawSampleWriteInput[]=[];for(const raw of input.rows){let row=baseSampleInput(raw);if(raw.vendorId){if(!vendorCache.has(raw.vendorId))vendorCache.set(raw.vendorId,masterRepository.findVendorById(raw.vendorId));const v=await vendorCache.get(raw.vendorId)!;if(!v)throw notFound(`Vendor ${raw.vendorId} tidak ditemukan.`);row={...row,vendorSnapshot:raw.vendorSnapshot??v.name};}if(raw.sourceId){if(!sourceCache.has(raw.sourceId))sourceCache.set(raw.sourceId,masterRepository.findSourceById(raw.sourceId));const s=await sourceCache.get(raw.sourceId)!;if(!s)throw notFound(`Source ${raw.sourceId} tidak ditemukan.`);if(s.materialKind!==raw.materialKind)throw new AppError(400,'MATERIAL_KIND_MISMATCH',`Source untuk ${raw.sampleId} tidak sesuai material.`);row={...row,sourceSnapshot:raw.sourceSnapshot??s.name};}rows.push(row);}const result=await repository.importRawSamples(rows,input.mode);await repository.appendAudit({actorUserId:actor.userId,actorRoleSnapshot:actor.role,action:'IMPORT',entityType:'RAW_SAMPLE',entityId:'BULK',afterJson:{...result,mode:input.mode,rowCount:rows.length},reason:input.reason,requestId});return result;},
