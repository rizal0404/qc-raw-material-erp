...
export function createVendorOperationService(repository: VendorShiftReportRepository, master: MasterRepository) {
  ...
  async function validateVendor(vendorId:string) {
  ...
  async function enrichAssignments(vendorId:string, shiftCode:ShiftCode, assignments:ShiftAssignmentInput[]) {
  ...
  async function writeRecord(principal:AuthPrincipal, input:CreateShiftReportRequest):Promise<ShiftReportWriteRecord>{
    ...
    return {
      ...
      assignments:await enrichAssignments(vendorId,input.shiftCode,input.assignments),
    };
  }
  ...
  return {
    async getCurrent(principal:AuthPrincipal,query:{vendorId?:string | undefined;operationDate:string;shiftCode:ShiftCode}){
      const vendorId=effectiveVendor(principal,query.vendorId);
      return repository.getCurrent({vendorId,operationDate:query.operationDate,shiftCode:query.shiftCode});
    },
    ...
    async list(principal:AuthPrincipal,query:{vendorId?:string | undefined;operationDate?:string | undefined;shiftCode?:ShiftCode | undefined;status?:any;limit:number;offset:number}){
    ...
    async createDraft(principal:AuthPrincipal,input:CreateShiftReportRequest,requestId?:string){
      ...
      const current=await repository.getCurrent({vendorId:data.vendorId,operationDate:data.operationDate,shiftCode:data.shiftCode});
    ...
    async updateDraft(principal:AuthPrincipal,id:string,input:UpdateShiftReportDraftRequest,requestId?:string){
      ...
      const enriched=await enrichAssignments(before.vendorId,before.shiftCode,input.assignments);
    ...
    async getEffectiveSubmitted(principal:AuthPrincipal,query:{vendorId?:string | undefined;operationDate:string;shiftCode:ShiftCode}){
      const vendorId=effectiveVendor(principal,query.vendorId);
      return repository.getEffectiveSubmitted({vendorId,operationDate:query.operationDate,shiftCode:query.shiftCode});
    },
  };
}
