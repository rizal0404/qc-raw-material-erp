import type {
  CreateShiftReportRequest, CreateShiftReportRevisionRequest, ShiftCode, ShiftReport, ShiftReportStatus, UpdateShiftReportDraftRequest,
} from '@qc/contracts';
...
export function getCurrentShiftReport(input:{operationDate:string;shiftCode:ShiftCode;vendorId?:string}){
  return apiFetch<ShiftReportResponse>(`/vendor/shift-reports/current?${qs(input)}`);
}
export function listShiftReports(input:{vendorId?:string;operationDate?:string;shiftCode?:ShiftCode;status?:ShiftReportStatus;limit?:number;offset?:number}={}){
...
export function equipmentLookup(vendorId:string,type:'AM'|'AA'){return apiFetch<{ok:true;items:EquipmentLookupItem[]}>(`/lookups/equipment?${qs({vendorId,type})}`)}
