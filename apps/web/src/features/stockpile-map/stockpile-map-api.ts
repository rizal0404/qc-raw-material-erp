...
export function getStockpileMap(layoutId:string,lotStatus:StockpileLotStatusFilter,asOf:string){return apiFetch<StockpileMapResponse>(`/stockpile-map?layoutId=${encodeURIComponent(layoutId)}&lotStatus=${lotStatus}&asOf=${encodeURIComponent(asOf)}`);}
