...
export function WarehouseMap({data,colorMode,selectedLayerId,onSelectLayer,recPosition,onRecPositionChange}:{data:StockpileMapResponse;colorMode:ColorMode;selectedLayerId:string|null;onSelectLayer:(layer:StockpileLayer)=>void;recPosition:number;onRecPositionChange:(position:number)=>void}){
  ...
  const startDrag=(event:ReactPointerEvent<SVGRectElement>)=>{event.currentTarget.setPointerCapture(event.pointerId);onRecPositionChange(Math.round(pointerPosition(event)*100)/100);};
  const moveDrag=(event:ReactPointerEvent<SVGRectElement>)=>{if(event.currentTarget.hasPointerCapture(event.pointerId))onRecPositionChange(Math.round(pointerPosition(event)*100)/100);};
  ...
  return <div className="stockpile-map-scroll"><svg ref={svgRef} className="stockpile-map-svg" viewBox={`0 0 ${WIDTH} 360`} role="img" aria-label={`Peta mutu ${layout.name}`}>
    ...
    <g className="reclaimer-handle"><rect x={x(recPosition)-13} y={TOP-31} width="26" height={BOTTOM-TOP+62} rx="3" role="slider" aria-label={`Posisi REC ${positionLabel(recPosition,layout.postMarks)}`} aria-valuemin={0} aria-valuemax={layout.axisLength} aria-valuenow={recPosition} tabIndex={0} onPointerDown={startDrag} onPointerMove={moveDrag} onKeyDown={(event)=>{if(['ArrowLeft','ArrowRight'].includes(event.key)){event.preventDefault();keyMove(event.key);}}}/><text x={x(recPosition)} y={TOP-10} textAnchor="middle">REC</text></g>
  </svg></div>;
}

export function SelectedLayerDetails({layer,lot,onEdit,onEditLot,onToggleLot}:{layer:StockpileLayer|null;lot:StockpileLot|null;onEdit:()=>void;onEditLot:()=>void;onToggleLot:()=>void}){
