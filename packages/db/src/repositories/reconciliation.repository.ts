...
export function createReconciliationRepository(db:PostgresJsDatabase<typeof Schema>):ReconciliationRepository{
  ...
  async function assignmentRows(filter:{operationDate:string;shiftCode?:ShiftCode | undefined;crusherId?:string | undefined;vendorId?:string | undefined;materialKind?:MaterialKind | undefined;search?:string | undefined}){
    const rows=await db.execute(sql`
      WITH event_stats AS (
        ...
        SELECT assignment_id,count(*) FILTER (WHERE active=true)::int AS assigned_aa_count
        FROM loading_assignment_aas GROUP BY assignment_id
      ), alloc_stats AS (
        SELECT assignment_id,
          coalesce(sum(CASE WHEN mapping_status IN ('CONFIRMED','REVIEW_REQUIRED') THEN approved_retase ELSE 0 END),0)::int AS confirmed_reserved,
      ...
      SELECT la.id AS assignment_id,la.assignment_origin,la.report_id,r.version AS report_version,coalesce(r.status::text,'OPERATIONAL') AS report_status,la.operation_date,la.shift_code,
        ...
        coalesce(ast.assigned_aa_count,0) AS assigned_aa_count,coalesce(es.aa_with_dump_count,0) AS aa_with_dump_count,
      ...
      JOIN crushers c ON c.id=la.crusher_id JOIN vendors v ON v.id=la.vendor_id JOIN equipment am ON am.id=la.am_id
      ...
      WHERE la.operation_date=${filter.operationDate}::date AND la.status='ACTIVE'
        ...
        ${filter.crusherId?sql`AND la.crusher_id=${filter.crusherId}::uuid`:sql``}
  ...
  return {
    async listAssignments(filter){
      ...
      return rows.map(r=>{const observed=num(r.observed_retase),reserved=num(r.confirmed_reserved)+num(r.consumed_retase);return {assignmentId:str(r.assignment_id),assignmentOrigin:str(r.assignment_origin) as ReconciliationAssignmentRecord['assignmentOrigin'],reportId:nullable(r.report_id),reportVersion:r.report_version==null?null:num(r.report_version),reportStatus:str(r.report_status),operationDate:str(r.operation_date),shiftCode:str(r.shift_code) as ShiftCode,crusherId:str(r.crusher_id),crusherCode:str(r.crusher_code),crusherName:str(r.crusher_name),vendorId:str(r.vendor_id),vendorCode:str(r.vendor_code),vendorName:str(r.vendor_name),amId:str(r.am_id),amUnitNo:str(r.am_unit_no),sourceId:nullable(r.source_id),sourceCode:nullable(r.source_code),sourceName:nullable(r.source_name),blockSnapshot:nullable(r.block_snapshot),materialKind:str(r.material_kind) as MaterialKind,materialCategory:str(r.material_category),assignedAaCount:num(r.assigned_aa_count),aaWithDumpCount:num(r.aa_with_dump_count),observedRetase:observed,reservedRetase:reserved,consumedRetase:num(r.consumed_retase),remainingRetase:observed-reserved,reviewRequired:Boolean(r.review_required),allocations:alloc.get(str(r.assignment_id))??[]} satisfies ReconciliationAssignmentRecord;});
    },
    async getAssignment(assignmentId){
      ...
      return {assignmentId:str(row.assignment_id),assignmentOrigin:str(row.assignment_origin) as ReconciliationAssignmentRecord['assignmentOrigin'],reportId:nullable(row.report_id),reportVersion:row.report_version==null?null:num(row.report_version),reportStatus:str(row.report_status),operationDate:str(row.operation_date),shiftCode:str(row.shift_code) as ShiftCode,crusherId:str(row.crusher_id),crusherCode:str(row.crusher_code),crusherName:str(row.crusher_name),vendorId:str(row.vendor_id),vendorCode:str(row.vendor_code),vendorName:str(row.vendor_name),amId:str(row.am_id),amUnitNo:str(row.am_unit_no),sourceId:nullable(row.source_id),sourceCode:nullable(row.source_code),sourceName:nullable(row.source_name),blockSnapshot:nullable(row.block_snapshot),materialKind:str(row.material_kind) as MaterialKind,materialCategory:str(row.material_category),assignedAaCount:num(row.assigned_aa_count),aaWithDumpCount:num(row.aa_with_dump_count),observedRetase:observed,reservedRetase:reserved,consumedRetase:num(row.consumed_retase),remainingRetase:observed-reserved,reviewRequired:Boolean(row.review_required),allocations:alloc.get(assignmentId)??[]};
    },
    async listCandidates(assignmentId){
      const rows=await db.execute(sql`
        WITH a AS (
          SELECT la.id,la.operation_date,la.material_kind,la.vendor_id,v.name AS vendor_name,v.code AS vendor_code
          FROM loading_assignments la JOIN vendors v ON v.id=la.vendor_id WHERE la.id=${assignmentId}::uuid
        ), aliases AS (
    ...
    async listExceptions(filter){
      const rows=await db.execute(sql`
        ...
        WHERE re.operation_date=${filter.operationDate}::date AND re.event_type='DUMP' AND re.status IN ('EXCEPTION_UNASSIGNED','AMBIGUOUS')
          ${filter.shiftCode?sql`AND re.shift_code=${filter.shiftCode}`:sql``}${filter.crusherId?sql`AND re.crusher_id=${filter.crusherId}::uuid`:sql``}${filter.vendorId?sql`AND re.vendor_id=${filter.vendorId}::uuid`:sql``}
    ...
    async listExceptionAssignmentCandidates(eventId){
      const rows=await db.execute(sql`
        ...
        SELECT la.id AS assignment_id,la.assignment_origin,la.report_id,vsr.version AS report_version,la.shift_code,la.vendor_id,v.name AS vendor_name,
               ...
               la.crusher_id,c.name AS crusher_name,la.valid_from,la.valid_to,e.event_ts,sh.start_time AS shift_start,sh.end_time AS shift_end,sh.crosses_midnight,
        ...
        FROM e
        JOIN loading_assignments la ON la.operation_date=e.operation_date AND la.shift_code=e.shift_code AND la.crusher_id=e.crusher_id AND la.status='ACTIVE'
        JOIN shifts sh ON sh.code=la.shift_code AND sh.active=true
        LEFT JOIN vendor_shift_reports vsr ON vsr.id=la.report_id
        JOIN vendors v ON v.id=la.vendor_id JOIN equipment am ON am.id=la.am_id JOIN crushers c ON c.id=la.crusher_id
    ...
    async resolveExceptionEvent(input){
      await db.transaction(async(tx)=>{
        ...
        const assignmentRows=await tx.execute(sql`
          ...
          JOIN equipment am ON am.id=la.am_id JOIN crushers c ON c.id=la.crusher_id JOIN shifts sh ON sh.code=la.shift_code LEFT JOIN sources s ON s.id=la.source_id LEFT JOIN piles p ON p.id=la.pile_id
          ...
          WHERE la.id=${input.assignmentId}::uuid FOR UPDATE
        `);
        ...
        if(str(a.operation_date)!==str(e.operation_date)||str(a.shift_code)!==str(e.shift_code)||str(a.crusher_id)!==str(e.crusher_id))throw new Error('Assignment tidak sesuai operation date / shift / crusher event.');
    ...
    async updateAllocation(id,patch){
      ...
      if('sampleId'in patch)values.sampleId=patch.sampleId;if('approvedRetase'in patch)values.approvedRetase=patch.approvedRetase;if('candidateCount'in patch){values.candidateCount=patch.candidateCount;values.mappingStatus=(patch.candidateCount??0)>1?'AMBIGUOUS':'SUGGESTED';}if('note'in patch)values.note=patch.note;
    ...
    async confirmAllocation(id,input){
      await db.transaction(async(tx)=>{
        ...
        const reserveRows=await tx.execute(sql`SELECT coalesce(sum(CASE WHEN mapping_status IN ('CONFIRMED','REVIEW_REQUIRED') THEN approved_retase WHEN mapping_status='CONSUMED' THEN consumed_retase ELSE 0 END),0)::int AS reserved FROM qc_retase_allocations WHERE assignment_id=${str(a.assignment_id)}::uuid AND id<>${id}::uuid`);const reserved=num((reserveRows as unknown as Array<Record<string,unknown>>)[0]?.reserved);
